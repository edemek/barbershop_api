 ...
 */
class EService extends Model implements HasMedia
{
    ...
    {
        // Sauvegarder l'état actuel de skipPresenter
        $temporarySkipPresenter = $this->skipPresenter;

        // Sauter le presenter pour obtenir un modèle brut
        $this->skipPresenter(true);

        // Créer le modèle avec les données fournies
        $model = EService::create($data);

        // Rétablir l'état précédent de skipPresenter
        $this->skipPresenter($temporarySkipPresenter);

        // Mettre à jour les relations si nécessaire
        $model = $this->updateRelations($model, $data);

        // Sauvegarder le modèle après la mise à jour des relations
        $model->save();

        // Retourner le résultat du modèle (vous pouvez personnaliser cela si nécessaire)
